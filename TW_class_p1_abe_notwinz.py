...
class TW_abe ():
    def __init__(self, ma=2, mb=2, mc=2, kx=3, ky=3, kz=3, solver = 'osqp'):
        ...
        self.outputs_twin = list(product(range(ma),range(ma), range(mb), range(mb),range(mc)))
    ...
    def canonicalize(self, a, at, b, bt, c, x, xt, y, yt, z):
        if (a, x) > (at, xt):
            a, at = at, a
            x, xt = xt, x
        if (b, y) > (bt, yt):
            b, bt = bt, b
            y, yt = yt, y
        return (a, at, b, bt, c, x, xt, y, yt, z)
    ...
    def build_index_map(self):
        ...
        for (a, at, b, bt, c) in self.outputs_twin:
            for (x, xt, y, yt, z) in self.inputs_twin:

                key = (a, at, b, bt, c, x, xt, y, yt, z)
    ...
    def get_P_twin(self, P_twin, index_map,
               a, at, b, bt, c,
               x, xt, y, yt, z):
        return P_twin[index_map[(a, at, b, bt, c, x, xt, y, yt, z)]]
    ...
    def add_ns_constraints(self, P_twin, problem, index_map):
        ...
        for x in range(self.kx):
            for a in range(self.ma):
                ref_terms = [
                    self.get_P_twin(P_twin, index_map, a, a_twin, b, b_twin,c,
                                     x,0, 0,0, 0)
                    for a_twin, b, b_twin, c in product(range(self.ma), range(self.mb), range(self.mb), range(self.mc))
                ]
        ...
        for a_twin in range(self.ma):
            for xt in range(self.kx):
                ref_terms = [
                    self.get_P_twin(P_twin, index_map, a, a_twin, b, b_twin,c,
                                     0,xt, 0,0, 0)
                    for a, b, b_twin, c in product(range(self.ma), range(self.mb), range(self.mb), range(self.mc))
                ]
                ref = pc.sum(ref_terms)
        ...
        for y in range(self.ky):
            for b in range(self.mb):
                ref_terms = [
                    self.get_P_twin(P_twin, index_map, a, a_twin, b, b_twin,c,
                                     0,0, y,0, 0)
                    for a,a_twin, b_twin, c in product(range(self.ma), range(self.ma), range(self.mb), range(self.mc))
                ]
                ref = pc.sum(ref_terms)
        ...
        for yt in range(self.ky):
            for b_twin in range(self.mb):
                ref_terms = [
                    self.get_P_twin(P_twin, index_map, a, a_twin, b, b_twin,c,
                                     0,0, 0,yt, 0)
                    for a,a_twin, b, c in product(range(self.ma), range(self.ma), range(self.mb), range(self.mc))
                ]
                ref = pc.sum(ref_terms)
    ...
    def normalization_twin(self, P_twin, problem, index_map):
        for x, x_twin, y, y_twin, z in self.inputs_twin:
            ...
            for a, a_twin, b, b_twin, c in self.outputs_twin:
                val = self.get_P_twin(
                    P_twin, index_map,
                    a, a_twin, b, b_twin, c,
                    x, x_twin, y, y_twin, z
    ...
    def solve_k3_2(self):
        ...
        I = self.new_inequality_k3(P)

        problem.set_objective('max', I)   # inequality is ≥ M(d-1)
        problem.solve(solver=self.solver)  # SCS/ECOS también funcionan
